...
const ListPagination = ({
  ...
  const createPaginationNumberList = () => {
    if (paginationRange) {
      return paginationRange.map((pageNumber, index) => {
        ...
        return (
          <Avatar
            style={style.avatarMargins}
            sx={isAvatarSelected(pageNumber)}
            key={index}
          >
            <Link style={style.Link} key={index} to={"/gallery/" + pageNumber}>
              {pageNumber}
            </Link>
          </Avatar>
        );
  ...
  return (
    <div style={style.paginationContainer}>
      <Link
        style={{ ...style.Link, ...style.marginLink }}
        to={"/gallery/" + String(currentPage - 1)}
        onClick={isPrevIconButtonVisible}
      >
        <ChevronLeftIcon />
      </Link>
      <List style={style.listStyle}>{createPaginationNumberList()}</List>
      <Link
        style={{ ...style.Link, ...style.marginLink }}
        to={"/gallery/" + String(currentPage + 1)}
        onClick={isNextIconButtonVisible}
      >
        <ChevronRightIcon />
      </Link>
    </div>
